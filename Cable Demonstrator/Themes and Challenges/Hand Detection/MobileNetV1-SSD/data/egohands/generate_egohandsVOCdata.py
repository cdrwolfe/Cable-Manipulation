...
This script basically allows you to build a new set images built from the egohands VOC dataset, an egohands formatted setup 
in the pascal VOC style. It goes through and builds individual training, validation and test sets (files) which link to the 
annotation and image filenames / labels.

To run use: python generate_egohandsVOCdata.py ego-model-labels.txt
...
def main(filename):
    ...
    # get image names
    for filename in os.listdir("egohandsVOC/JPEGImages"):
    ...
    for img in imgnames:
        annote = "egohandsVOC/Annotations/" + img + '.xml'
    ...
    with open("egohandsVOC/ImageSets/Main/train.txt", 'w') as outfile:
        for name in train_list:
            outfile.write(name + "\n")
    with open("egohandsVOC/ImageSets/Main/val.txt", 'w') as outfile:
        for name in val_list:
            outfile.write(name + "\n")
    with open("egohandsVOC/ImageSets/Main/trainval.txt", 'w') as outfile:
        for name in train_list:
            outfile.write(name + "\n")
        for name in val_list:
            outfile.write(name + "\n")
    with open("egohandsVOC/ImageSets/Main/test.txt", 'w') as outfile:
        for name in test_list:
            outfile.write(name + "\n")

    # create the individiual files for each label
    for label in labels:
        with open("egohandsVOC/ImageSets/Main/"+ label +"_train.txt", 'w') as outfile:
            for name in train_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
                    outfile.write(name + " -1\n")
        with open("egohandsVOC/ImageSets/Main/"+ label +"_val.txt", 'w') as outfile:
            for name in val_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
                    outfile.write(name + " -1\n")
        with open("egohandsVOC/ImageSets/Main/"+ label +"_test.txt", 'w') as outfile:
            for name in test_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
...
if __name__=='__main__':
    if len(sys.argv) < 2:
        print("usage: python generate_egohansdVOCdata.py <labelfile>")
        exit()
    main(sys.argv[1])