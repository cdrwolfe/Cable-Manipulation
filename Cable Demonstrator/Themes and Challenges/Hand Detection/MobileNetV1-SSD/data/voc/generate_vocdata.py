...
This script basically allows you to build a new set images built from the VOC2012 dataset. It goes through 
and builds individual training, validation and test sets (files) which link to the annotation and image filenames / labels.

To run use: python generate_vocdata.py ./VOC2012/voc-model-labels.txt
...
def main(filename):
    ...
    # get image names
    for filename in os.listdir("./VOC2012/JPEGImages"):
    ...
    for img in imgnames:
        annote = "VOC2012/Annotations/" + img + '.xml'
    ...
    create_folder("./VOC2012/ImageSets/Main/")
    with open("./VOC2012/ImageSets/Main/train.txt", 'w') as outfile:
        for name in train_list:
            outfile.write(name + "\n")
    with open("./VOC2012/ImageSets/Main/val.txt", 'w') as outfile:
        for name in val_list:
            outfile.write(name + "\n")
    with open("./VOC2012/ImageSets/Main/trainval.txt", 'w') as outfile:
        for name in train_list:
            outfile.write(name + "\n")
        for name in val_list:
            outfile.write(name + "\n")
    with open("./VOC2012/ImageSets/Main/test.txt", 'w') as outfile:
        for name in test_list:
            outfile.write(name + "\n")

    # create the individiual files for each label
    for label in labels:
        with open("./VOC2012/ImageSets/Main/"+ label +"_train.txt", 'w') as outfile:
            for name in train_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
                    outfile.write(name + " -1\n")
        with open("./VOC2012/ImageSets/Main/"+ label +"_val.txt", 'w') as outfile:
            for name in val_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
                    outfile.write(name + " -1\n")
        with open("./VOC2012/ImageSets/Main/"+ label +"_test.txt", 'w') as outfile:
            for name in test_list:
                if label in annotations[name]:
                    outfile.write(name + " 1\n")
                else:
